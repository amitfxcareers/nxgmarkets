<?php include_once('./include/header.php') ?>
<!-- Banner -->
<section class="inner-banner" style="background-image: url(images/forex_banner.webp);">
   <div class="container">
      <div class="row">
         <div class="col-md-6">
            <div class="banner_content ">
               <h2 class="text-white">
                  Forex
               </h2>
               <p class="text-white">
                  Welcome to NXG Markets, your entry to the world’s largest and most actively traded online financial market, Forex. Explore the benefits of trading forex with us.
               </p>
               <a href="https://my.nxgmarkets.com/auth/register" class="theme-bttn"><i class="fa fa-user" aria-hidden="true"></i>Register</a>
            </div>
         </div>
      </div>
   </div>
</section>
<!-- Banner -->
<!-- Forex Intro -->
<section class="forex-intro">
   <div class="container">
      <div class="row align-items-center">
         <div class="col-md-6">
            <h5 class="text-black fw-600">
               Welcome to NXG Markets
            </h5>
            <p>
               Experience the dynamic world of forex trading with NXG Markets. Access diverse currency pairs, low spreads, and secure transactions. Our user-friendly platform and advanced tools empower traders to succeed in the forex market.
            </p>
            <h5 class="text-black fw-600">
               Trade Global Currencies with the Best Trading Conditions and Tools
            </h5>
            <h2 class="text-primary fw-800">
               What is Forex?
            </h2>
            <p>
               Forex, or the foreign exchange market, is where global currency trading occurs. It provides opportunities for traders to capitalize on exchange rate fluctuations, with a daily trade volume of $6.5 trillion and offering significant potential for both beginners and experienced traders.
            </p>
            <div class="mt-4">
               <a href="https://my.nxgmarkets.com/auth/register" class="theme-bttn"><i class="fa fa-user" aria-hidden="true"></i>Register Now</a>
            </div>
         </div>
         <div class="col-md-6">
            <lottie-player src="images/lottie-json/forex.json" style="width: 100%; height: 450px" background="transparent" speed="1" direction="1" mode="normal" loop="" autoplay=""></lottie-player>
         </div>
      </div>
   </div>
</section>
<!-- Forex Intro -->
<!-- Forex Table -->
<section class="forex-table">
   <div class="container">
      <div class="row">
         <div class="col-md-12">
            <table class="forex_table">
               <thead>
                  <tr class="uael-table-row">
                     <th>
                        Instrument symbol
                     </th>
                     <th>
                        Instrument name
                     </th>
                     <th>
                        Nominal value of the lot
                     </th>
                     <th>
                        Size of one pip
                     </th>
                     <th>
                        Trading hours (GMT+0)
                     </th>
                  </tr>
               </thead>
               <tbody>
                  <tr class="uael-table-row">
                     <td>
                        AUDCAD
                     </td>
                     <td>
                        Australian Dollar vs Canadian Dollar
                     </td>
                     <td>
                        AUD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        AUDCHF
                     </td>
                     <td>
                        Australian Dollar vs Swiss Franc
                     </td>
                     <td>
                        AUD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        AUDJPY
                     </td>
                     <td>
                        Australian Dollar vs Japanese Yen
                     </td>
                     <td>
                        AUD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        AUDNZD
                     </td>
                     <td>
                        Australian Dollar vs New Zealand Dollar
                     </td>
                     <td>
                        AUD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        AUDUSD
                     </td>
                     <td>
                        Australian Dollar vs US Dollar
                     </td>
                     <td>
                        AUD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        CADCHF
                     </td>
                     <td>
                        Canadian Dollar vs Swiss Franc
                     </td>
                     <td>
                        CAD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        CADJPY
                     </td>
                     <td>
                        Canadian Dollar vs Japanese Yen
                     </td>
                     <td>
                        CAD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        CHFJPY
                     </td>
                     <td>
                        Swiss Franc vs Japanese Yen
                     </td>
                     <td>
                        CHF 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURAUD
                     </td>
                     <td>
                        Euro vs Australian Dollar
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURCAD
                     </td>
                     <td>
                        Euro vs Canadian Dollar
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURCHF
                     </td>
                     <td>
                        Euro vs Swiss Franc
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURGBP
                     </td>
                     <td>
                        Euro vs British Pounds
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURJPY
                     </td>
                     <td>
                        Euro vs Japanese Yen
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURNOK
                     </td>
                     <td>
                        Euro vs Norwegian Krone
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURNZD
                     </td>
                     <td>
                        Euro vs New Zealand Dollar
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURPLN
                     </td>
                     <td>
                        Euro vs Polish Zloty
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURSEK
                     </td>
                     <td>
                        Euro vs Swedish Krona
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURTRY
                     </td>
                     <td>
                        Euro vs Turkish Lira
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        EURUSD
                     </td>
                     <td>
                        Euro vs US Dollar
                     </td>
                     <td>
                        EUR 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        GBPAUD
                     </td>
                     <td>
                        British Pounds vs Australian Dolla
                     </td>
                     <td>
                        GBP 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        GBPCAD
                     </td>
                     <td>
                        British Pounds vs Canadian Dollar
                     </td>
                     <td>
                        GBP 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        GBPCHF
                     </td>
                     <td>
                        British Pounds vs Swiss Franc
                     </td>
                     <td>
                        GBP 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        GBPJPY
                     </td>
                     <td>
                        British Pounds vs Japanese Yen
                     </td>
                     <td>
                        GBP 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        GBPNZD
                     </td>
                     <td>
                        British Pounds vs New Zealand Dollar
                     </td>
                     <td>
                        GBP 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        GBPUSD
                     </td>
                     <td>
                        British Pounds vs US Dollar
                     </td>
                     <td>
                        GBP 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        NZDCAD
                     </td>
                     <td>
                        New Zealand Dollar vs Canadian Dollar
                     </td>
                     <td>
                        NZD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        NZDCHF
                     </td>
                     <td>
                        New Zealand Dollar vs Swiss Franc
                     </td>
                     <td>
                        NZD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        NZDJPY
                     </td>
                     <td>
                        New Zealand Dollar vs Japanese Yen
                     </td>
                     <td>
                        NZD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        NZDUSD
                     </td>
                     <td>
                        New Zealand Dollar vs US Dollar
                     </td>
                     <td>
                        NZD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDCAD
                     </td>
                     <td>
                        US Dollar vs Canadian Dollar
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDCHF
                     </td>
                     <td>
                        US Dollar vs Swiss Franc
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDHKD
                     </td>
                     <td>
                        US Dollar vs Hong Kong Dollar
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDJPY
                     </td>
                     <td>
                        US Dollar vs Japanese Yen
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDMXN
                     </td>
                     <td>
                        US Dollar vs Mexican Peso
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDNOK
                     </td>
                     <td>
                        US Dollar vs Norwegian Krone
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDPLN
                     </td>
                     <td>
                        US Dollar vs Polish Zloty
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDRUB
                     </td>
                     <td>
                        US Dollar vs Russian Rubble
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDSEK
                     </td>
                     <td>
                        US Dollar vs Swedish Krona
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDTRY
                     </td>
                     <td>
                        US Dollar vs Turkish Lira
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-23:59:59
                     </td>
                  </tr>
                  <tr class="uael-table-row">
                     <td>
                        USDZAR
                     </td>
                     <td>
                        US Dollar vs South African Rand
                     </td>
                     <td>
                        USD 100 000
                     </td>
                     <td>
                        0.01
                     </td>
                     <td>
                        00:00-21:00 22:00-23:59:59
                     </td>
                  </tr>
               </tbody>
            </table>
         </div>
      </div>
   </div>
</section>
<!-- Forex Table -->
<!-- Why Trade Forex with NXG Markets? -->
<section class="why-trade-with-nxg">
   <div class="container">
      <div class="row align-items-center">
         <div class="col-md-6">
            <div class="mb-5">
               <h4 class="text-white">
                  Trading Instruments
               </h4>
               <h2 class="text-white">
                  Why Trade Forex with NXG Markets?
               </h2>
            </div>
            <div class="icon-box">
               <div class="icon">
                  <img src="images/competitive-trading-conditions.png" class="img-fluid">
               </div>
               <h4 class="title">Competitive Trading Conditions</h4>
               <p class="description">Benefit from tight spreads, low commissions, and lightning-fast order execution, At Sub Second level. We prioritize competitive pricing to optimize your trading strategies.</p>
            </div>
            <div class="icon-box">
               <div class="icon">
                  <img src="images/wide-range-of-currency-pairs.png" class="img-fluid">
               </div>
               <h4 class="title">Wide Range of Currency Pairs</h4>
               <p class="description">Explore 40+ currency pairs, including major global currencies with high liquidity. Diversify your portfolio and seize opportunities in various market movements.</p>
            </div>
            <div class="icon-box">
               <div class="icon">
                  <img src="images/advanced-trading-platforms.png" class="img-fluid">
               </div>
               <h4 class="title">Advanced Trading Platforms</h4>
               <p class="description">Choose from MT5, cTrader. Trade seamlessly on the platform that suits your style and preferences.</p>
            </div>
            <div class="icon-box">
               <div class="icon">
                  <img src="images/secure-and-convenient-transactions.png" class="img-fluid">
               </div>
               <h4 class="title">Secure and Convenient Transactions</h4>
               <p class="description">Enjoy multiple deposit and withdrawal options, ensuring secure and hassle-free fund management.</p>
            </div>
         </div>
         <div class="col-md-6 text-center">
            <img src="images/why-trade-forex.png" class="img-fluid" width="80%">
         </div>
      </div>
      <div class="row mt-4">
         <div class="col-md-12">
            <div class="text-center">
               <?php include_once('./include/register-now-button.php') ?>
            </div>
         </div>
      </div>
   </div>
</section>
<!-- Why Trade Forex with NXG Markets? -->
<!-- Trade Forex -->
<section class="trade-section">
   <div class="container">
      <div class="row">
         <div class="col-md-12">
            <h2 class="main-heading">Trade Forex</h2>
            <p class="text-center">
               Forex is the worldwide marketplace for currency exchange, providing traders with opportunities to profit from fluctuations in exchange rates. With a daily trade volume of $6.5 trillion, Forex offers substantial potential for both novice and professional traders. Open your trading account with NXG Markets and trade the forex markets securely.
            </p>
         </div>
         <div class="col-md-12">
            <div class="trade-menu">
               <ul class="trade-list">
                  <li>
                     <div class="image-boxx text-center">
                        <a href="forex.php">
                           <span class="active">
                           <img src="images/forex.webp" class="img-fluid">
                           </span>
                           <h4>Forex</h4>
                        </a>
                     </div>
                  </li>
                  <li>
                     <div class="image-boxx text-center">
                        <a href="indices.php">
                           <span>
                           <img src="images/indices.webp" class="img-fluid">
                           </span>
                           <h4>Indices</h4>
                        </a>
                     </div>
                  </li>
                  <li>
                     <div class="image-boxx text-center">
                        <a href="commodities.php">
                           <span>
                           <img src="images/comm.webp" class="img-fluid">
                           </span>
                           <h4>Commodities</h4>
                        </a>
                     </div>
                  </li>
                  <li>
                     <div class="image-boxx text-center">
                        <a href="stocks.php">
                           <span>
                           <img src="images/stocks.webp" class="img-fluid">
                           </span>
                           <h4>Stocks</h4>
                        </a>
                     </div>
                  </li>
                  <li>
                     <div class="image-boxx text-center">
                        <a href="cryptocurrencies.php">
                           <span>
                           <img src="images/cryptos.webp" class="img-fluid">
                           </span>
                           <h4>Cryptos</h4>
                        </a>
                     </div>
                  </li>
               </ul>
            </div>
         </div>
      </div>
   </div>
</section>
<!-- Trade Forex -->
<?php include_once('./include/why-nxg-markets.php') ?>
<?php include_once('./include/multi-language-assistance.php') ?>
<?php include_once('./include/footer.php') ?>